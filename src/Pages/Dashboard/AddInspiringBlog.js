...
const AddInspiringBlog = () => {
          ...
          fetch("http://localhost:5000/inspiringBlog", {
            ...
            .then((insertedBlog) => {
              if (insertedBlog.insertedId) {
                toast.success("Inspiring stories Blog added successfully");
                reset();
              } else {
  ...
  return (
    <div className="my-2">
      <h1 className="text-2xl  font-semibold mb-4">Inspiring Stories</h1>
      <form onSubmit={handleSubmit(onSubmit)}>
        {/* <label className="label text-center">
          <span className="label-text  mx-auto ">Blog Title Name</span>
        </label> */}
        <input
          type="text"
        ...
        <label className="label">
          {errors.blogTitleName?.type === "required" && (
            <span className="label-text-alt text-red-500">
              {errors.blogTitleName.message}
            </span>
          )}
        </label>
...
{/* 
        ...
        <input
          className="btn w-full max-w-xs text-white hover:bg-primary bg-primary"
          type="submit"
          value="Add Blog"
        />
      </form>
...
export default AddInspiringBlog;
